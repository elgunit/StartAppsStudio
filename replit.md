...
The desktop landing page is served in 10 languages: English (default), Azerbaijani, Turkish, Russian, Simplified Chinese, French, Spanish, German, Ukrainian, and Italian.
...
- **Dictionaries**: `server/i18n/strings/<code>.json`, keyed by exact English source strings. Regenerate the source inventory with `npx tsx scripts/build-i18n-source.ts` (reports per-locale coverage). After editing the landing template, re-run it and top up any missing keys in all nine dictionaries.
...
- **SEO**: per-locale canonical, `og:locale`, hreflang alternates (incl. `x-default` → `/`), localized JSON-LD, supported-language and worldwide remote-delivery signals, locale URLs in the sitemap. Journal remains English-only. Run `npm run check:seo` to audit the public locale contract.
...
- `https://<prod-domain>/llms.txt` and `https://<prod-domain>/llms-full.txt` — machine-readable studio, package, language coverage, and worldwide remote-delivery summaries
